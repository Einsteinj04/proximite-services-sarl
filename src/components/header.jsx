    ...
    <>
      <nav
        ...
        className={`absolute sm:top-10 top-0  w-[80%] transition-all mx-auto duration-300 z-50 px-10 py-2 shadow-md left-1/2 -translate-x-1/2 
          `}
      >
        <div className='w-full md:flex items-cente hidden'>
          ...
          <div className='md:w-full md:max-w-full max-w-10 flex justify-around '>
            ...
            <div className='md:hidden block '>
              <IconButton onClick={handleOpenSideBar}>
                <MenuIcon className={` text-[19px] base:text-[22px] sm:text-[25px]  ${isScrolled ? 'text-slate-600' : 'text-slate-500'}`} />
              </IconButton>
            </div>
          </div>
        </div>
      ...
      <nav ref={sideBarRef} className='fixed z-[2000] top-0 right-0 left-0 bottom-0 w-full h-full max-h-full transition-all ease-out duration-300 md:hidden translate-y-[-100vh] bg-[var(--primary)] px-10 py-2'>
        <div className='flex'>
          <div className='w-full flex max-w-40 sm:justify-center justify-start items-center'>
            ...
            {/* <Image alt='Welcome to BuildingPlans' src={ASSETS.LOGO} className='w-full max-h-28' /> */}
            Logo
            </Link>
          </div>
